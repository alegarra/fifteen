...
! integer(i8)        ::     default_hash_size=8192,& !AL
 integer(i8)        ::     default_hash_size= 33554432,& !AL
 ...
 real           ::     default_conv=1e-10,&   ! stopping criterion
                       ...
                       maxfill=.90,&          ! max fill of hash table
...
!
 if (present(max_elem)) then
        x%nel=max_elem 
        x%nel=2** (1+ int(log(real(x%nel-1,r8))/log(2d0))) ! AL that is, find the closest, higher, power of two
	                                                   ! 5 -> 8 ; 8 -> 8
...
!     Matrix full, copy to a matrix hash_incr times larger
       ...
       print'(a,i0,a,i0,a,f10.4)','hash matrix increased from ',x%nel,' to ',int(hash_incr*x%nel),&
                                  ' % filled: ',real(x%filled,r8)/x%nel
       !call flush()                                  
       ! Move y to x
       call reset(x)
       x%n=y%n; x%nel=y%nel;x%filled=y%filled;x%status=y%status;x%xind=>y%xind; x%xreal=>y%yreal
...
! solves x * sol = rs for sparse-hash matrix x assumed upper-stored 
 ...
 if (x%storage_type=='f') then 
    print*,'SOLVE_SPARSE_HASHM: not yet implemented for full-stored matrix'
    stop
 endif 
 if (default_zerosol) sol=0
 ...
 select case (x%status)
     case (1)           !hash ready, needs sorting
        call clusterr(x%x,x%nel,3_i8,x%filled)
        call sortqr(x%x,x%nel,3_i8,x%filled,3_i8,(/1_i8,2_i8/),2_i8)
        x%status=2
...
! mode='internal' prints internal representation
 ...
 if (present(mode)) then
    if (mode(1:3)=='int') then
       do i=1,x%nel
          if (x%x(1,i) /= 0) then
             print '(2i8,f30.18)',int(x%x(1,i)),int(x%x(2,i)),x%x(3,i)
          endif
       enddo
       return
    endif
...
! y=x(i1:i2:step,j1:j2:step) for dense matrices
  type (densem):: x,y
  INTEGER(i8) :: i1,i2,j1,j2
  integer(i8),optional::step
  integer(i8) :: st
...
! y=x(i1:i2:step,j1:j2:step) for hash matrices
  type (sparse_hashm):: x,y
  INTEGER(i8),INTENT(IN) :: i1,i2,j1,j2
  INTEGER(i8) :: i,ix,jy
  integer(i8),optional::step
  integer(i8) :: st
 ...
 !
  ...
  call zerom(y,(i2-i1)/st+1,storage_type=x%storage_type)
   do i=1,x%nel
          if (x%x(1,i) /= 0) then
             ix=x%x(1,i)
             jy=x%x(2,i)
...
! tr=trace(x,y) for dense matrices
  ...
  if (x%storage_type=='f' .or. y%storage_type=='f') then 
     print*,'TRACE_HASHM: not yet implemented for full-stored matrix' 
     stop
  endif
  if (x%n /=y%n) then
     PRINT*,'TRACE_HASHM: Matrices of different size'
     stop
  end if
...
! y=trace(x,y(i1:i2:step,j1:j2:step)) for dense matrices
 ...
 !
  ...
  if (x%n /=(i2-i1)/st+1) then
     PRINT*,'BLOCKTRACE_DENSEM: Matrices of different size'
     stop
  end if
...
! y=trace(x,y(i1:i2:step,j1:j2:step)) for hash matrices
  ...
  INTEGER(i8),INTENT(IN) :: i1,i2,j1,j2
  real (r8)::tr
  INTEGER(i8) :: i,j,k,k1,k2,l,l1,l2,st
  integer(i8),optional::step
 ...
 !y
  ...
  if (x%storage_type=='f' .or. y%storage_type=='f') then 
     PRINT*,'BLOCKTRACE_HASHM: not yet implemented for full-stored matrix'
     stop
  endif
  if (i2-i1 /= j2-j1) then
...
! y=trace(x,y(i1:i2:step,j1:j2:step)) for ija matrices
  ...
  INTEGER(i8),INTENT(IN) :: i1,i2,j1,j2
  real (r8)::tr
  INTEGER(i8) :: i,j,k,k1,k2,l,l1,l2,m,st
  integer(i8),optional::step
...
! q=x'Ax for hash matrices
  ...
  if (a%storage_type=='f') then 
     PRINT*,'QUADR_HASHM: not yet implemented for full-stored matrix'
     stop
  endif 
  if (size(x) /=a%n .or. size(y) /=a%n ) then
...
! As solve_dense_gs but for summetric dense upper-stored matrix
 ...
 integer(i8) :: round,i,j
 
 round=0
 do
    eps=0; ss=0; round=round+1
    do i=1,n
       if (lhs(pos_symm_sp(i,i,n)).eq.0) cycle
       sum=0
       do j=1,n
          sum=sum+lhs(pos_symm_sp(i,j,n))*sol(j)
       enddo
       solnew=sol(i)+default_relax*(rhs(i)-sum)/lhs(pos_symm_sp(i,i,n))
       eps=eps+ (sol(i)-solnew)**2; ss=ss+solnew*solnew
       sol(i)=solnew
    end do
    eps=eps/ss
...
! As solve_dense_gs but for sparse hash assumed upper-stored matrix
 ...
 real (rh) :: lhs(:,:)
 real (rh) :: rhs(n),sol(n),solnew,eps,ss,sum
 integer(i8) :: round,i,j,l1,l2,row
 integer(i8):: ia(n+1)   !row pointer as IA in IJA format
...
! As solve_dense_gs but for sparse ija assumed upper-stored matrix
 integer(i8) :: n,nel,ia(:),ja(:)
 real (r8) :: a(:)
 real (rh) ::rhs(n),sol(n),solnew,eps,ss,sum
 integer(i8) :: i,j,round
 real (rh):: diag(n),rhsadj(n) !diagonals and adjusted right hand sides
...
do i=1,x%nel
   if (x%x(1,i).ne.0) then
      call addm(x%x(3,i),int(x%x(1,i),i8),int(x%x(2,i),i8),y)
      if (int(x%x(1,i),i8) /= int(x%x(2,i),i8) .and. x%storage_type=='u') &
                     call addm(x%x(3,i),int(x%x(2,i),i8),int(x%x(1,i),i8),y)
   endif
enddo
end subroutine
...
! converts symmetric hash matrix x into ija matrix y
 ...
 if (x%storage_type=='f') then 
    print*,'CONVERT_HASH_IJA: not yet implemented for full-stored hash matrix'
    stop
 endif
 ...
 type (sparse_ija), intent(inout) :: y
 type (sparse_hashm), intent(in) :: x
 integer(i8),optional::conv_type
 integer(i8):: i,j,k,l,row(x%n),conv_type_o
 real (rh) :: zero=0


   if (x%storage_type=='f') then
      print*,'CONVERT_HASH_IJA_GENERAL: not yet implemented for full-stored matrix'
      stop
   endif
   conv_type_o=0
...
subroutine convert_densem_half(y,x)
! converts dense matrix x into dense_symm y
...
select case(x%status)
   case(2)	! matrix sorted, copy by conversion to ija and back
      ...
      call reset(scr)
   case(1) 
     call zerom(y,x%n,storage_type=x%storage_type)
     deallocate(y%x)
     y%n=x%n
     y%nel=x%nel
     y%filled=x%filled
...
subroutine hashm_upper_full(y,x)
! convert upper to full hash matrix
type (sparse_hashm), intent(inout) :: y
type (sparse_hashm), intent(in) :: x
integer(i8) :: k
!
if (x%storage_type/='u') then 
   print*,'HASHM_UPPER_FULL: original matrix not upper'
   stop
endif 
call zerom(y,x%n,storage_type='f')
do k=1,x%nel
   if (x%x(1,k) /= 0) then
       call addm(x%x(3,k),int(x%x(1,k),i8),int(x%x(2,k),i8),y)
       if (int(x%x(1,k),i8)<int(x%x(2,k),i8) ) &
          call addm(x%x(3,k),int(x%x(2,k),i8),int(x%x(1,k),i8),y)
   endif
enddo
end subroutine

subroutine hashm_full_upper(y,x)
! convert full to upper hash matrix
type (sparse_hashm), intent(inout) :: y
type (sparse_hashm), intent(in) :: x
integer(i8) :: k
!
if (x%storage_type/='f') then
   print*,'HASHM_FULL_UPPER: original matrix not full storage'
   stop
endif
call zerom(y,x%n,storage_type='u')
do k=1,x%nel
   if (x%x(1,k) /= 0) then
       if (int(x%x(2,k),i8)>=int(x%x(1,k),i8) ) &
          call addm(x%x(3,k),int(x%x(1,k),i8),int(x%x(2,k),i8),y)
   endif
enddo
end subroutine
